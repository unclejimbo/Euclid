...
TEST_CASE("Analysis, Segmentation", "[analysis][segmentation]")
{
    ...
    SECTION("random walk segmentation of mesh")
    {
        ...
        std::vector<unsigned char> colors(segments.size() * 3);
        for (size_t i = 0; i < segments.size(); ++i) {
            auto id = sid[segments[i]];
            colors[3 * i + 0] = color_set[3 * id + 0];
            colors[3 * i + 1] = color_set[3 * id + 1];
            colors[3 * i + 2] = color_set[3 * id + 2];
        }
        std::string fout(TMP_DIR);
        fout.append("random_walk_segment_mesh.ply");
        Euclid::write_ply<3>(
